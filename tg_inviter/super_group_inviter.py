...
def load_log():
    ...
    return pd.DataFrame(columns=[
        'user_id', 'username', 'invited', 'invite_datetime', 'is_member',
        'last_check_datetime', 'tries', 'error_message'
    ])
...
async def is_member(client, group, user_id, username):
    try:
        ...
        for p in result.users:
            if (str(p.id) == str(user_id)) or (username and p.username and p.username.lower() == username.lower()):
                return True
        return False
...
async def main():
    ...
    async with TelegramClient(SESSION_NAME, API_ID, API_HASH) as client:
        ...
        for i, user in enumerate(to_check):
            ...
            if not member:
                invite_candidates.append({'user_id': user_id, 'username': username, 'tries': tries})
            if i % 10 == 0:
                save_log(df_log)
        save_log(df_log)
        ...
        for i in range(0, len(invite_candidates), USERS_PER_BATCH):
            ...
            try:
                ...
                print(f'  ✓ Добавлено {len(users_tuples)} пользователей ({i+1}-{i+len(users_tuples)})')
