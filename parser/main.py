...
async def parse_chat(client, chat, users_dict):
    try:
        ...
        print(f"[!] Нет доступа к {chat} (приватный канал/группа). Добавьте себя в чат!")
        return
    ...
    try:
        async for msg in client.iter_messages(entity, limit=1000):
            # Автор поста
            if msg.sender_id and msg.sender_id not in users_dict and msg.sender_id not in admins:
                try:
...
async def main():
    ...
    if os.path.exists(PARSED_SOURCES_FILE) or os.path.exists(USERS_CSV):
        ...
        if MODE == 'reset':
            ...
            print(f'Продолжаем. Уже обработано {len(parsed_sources)} из {len(all_sources_set)} источников.')
    else:
        parsed_sources = set()
        users_dict = {}
    ...
    async with TelegramClient(SESSION_NAME, int(API_ID), API_HASH) as client:
        for idx, raw_source in enumerate(sources):
            if raw_source in parsed_sources:
                print(f'[{idx+1}/{len(sources)}] Пропускаем уже обработанный источник: {raw_source}')
                continue
